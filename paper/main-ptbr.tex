...
\begin{figure}[H]
  \figplaceholder{4.5cm}{%
    Arquitetura IS-RAG: \textbf{esquerda} = pipeline de indexação
    (corpus $\to$ segmentação $\to$ embeddings duais + analisador
    cognitivo $\to$ PostgreSQL/pgvector);
    \textbf{direita} = recuperação em tempo de consulta
    (consulta $\to$ análise cognitiva $\to$ pontuação híbrida +
    boosting $\to$ resultados ranqueados).%
  }
...
\begin{figure}[H]
  \figplaceholder{4cm}{%
    Gráfico de barras: $\Delta$ NDCG@5 por consulta (Q2--Q10),
    colorido por família de esquema —
    \textsc{Force} (azul), \textsc{Container} (laranja),
    \textsc{Path} (verde), Cross (roxo).
    Linha de referência horizontal em $\Delta = 0$.%
  }
